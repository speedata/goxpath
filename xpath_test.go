...
func TestEval(t *testing.T) {
	...
	testdata := []struct {
		...
		{`count( /root/sub[3]/following-sibling::element() )  `, Sequence{4}},
		{`count(/root/sub[3]/following::element() )  `, Sequence{10}},
		{`count(/root/sub/self::sub)  `, Sequence{3}},
		{`boolean(/root/other[1]/(self::sub or self::other))  `, Sequence{true}},
		{`boolean(/root/other[1]/(self::sub or self::sub))  `, Sequence{false}},
		{`boolean(/root/sub[1]/(@foo and not(ends-with(@foo, 'z'))))  `, Sequence{false}},
		{`boolean(/root/sub[2]/(@foo and not(ends-with(@foo, 'z'))))  `, Sequence{true}},
		{`/root/sub[1]/string((@doesnotexist, @foo)[1])  `, Sequence{"baz"}},
		{`count(/root/sub/self::other)  `, Sequence{0}},
		{`count(/root/*/self::sub)  `, Sequence{3}},
		{`count(/root/self::root)  `, Sequence{1}},
		{`count(/root/sub/self::element())  `, Sequence{3}},
